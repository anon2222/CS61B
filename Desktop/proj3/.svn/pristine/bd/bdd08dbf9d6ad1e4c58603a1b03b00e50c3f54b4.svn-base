...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        _fringe.addAll(V0);
        LinkedList<Integer> postVisits = new LinkedList<Integer>();
        while (!_fringe.isEmpty()) {
            ...
            if (!marked(v)) {
                ...
                Iteration<Integer> rOrder = _G.successors(v);
                if (reverseSuccessors(v)) {
                    LinkedList<Integer> temp = new LinkedList<Integer>();
                    for (int i : rOrder) {
                        temp.addFirst(i);
                    }
                    rOrder = Iteration.iteration(temp);
                }
                for (int s : rOrder) {
        ...
        while (!postVisits.isEmpty()) {
            int element = postVisits.pollLast();
            if (!postVisit(element)) {
                return;
            }
        }
