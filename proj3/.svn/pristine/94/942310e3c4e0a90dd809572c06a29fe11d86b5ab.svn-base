...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 1; i < segment.size(); i += 1) {
            ...
            r = _map.getLabel(prev, curr);
            String road = r.toString();
            String dir = r.direction().fullName();
            if (pr == null || (pr.toString().equals(road)
        ...
        System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                          seq, r.toString(), r.direction().fullName(), len,
                          _map.getLabel(
